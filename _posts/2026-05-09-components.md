...
## ADC для потенціометрів *(optional)*

**ADS1115** — 16-бітний I²C ADC для читання крутилок. Або замість нього — **Arduino UNO**, який читає потенціометри напряму через аналогові входи і передає значення у FPGA через **UART**.
...
Аудіо вхід ─→ PCM1808 ──→ FPGA
                           │
Крутилки ──→ Arduino (або ADS1115) ──→ FPGA
```
...
- ADS1115 ADC модуль
- Arduino UNO *(optional — замість ADS1115)*
- 4× потенціометр 10kΩ
- Breadboard + дроти
