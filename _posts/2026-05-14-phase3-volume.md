...
Написав I²C master на Verilog. Підключив. Крутиш потенціометр — гучність не змінюється. Що робити?

I²C — це протокол. "Підключив і запрацювало" майже ніколи не буває з першого разу. Треба побачити **що реально летить по дротах** SDA і SCL. Для цього — **PulseView** (open-source логічний аналізатор): підключаєш до SDA/SCL і бачиш які байти відправляються, де ACK/NACK, де помилка в протоколі. Без нього дебажити I²C — це гадати навмання.
...
В цій схемі **FPGA є master** — вона ініціює запити. **Arduino є slave** — вона відповідає:

```
FPGA → "дай мені значення потенціометра" → Arduino
FPGA ←        "ось значення: 512"        ← Arduino
```
...
Arduino — 5V логіка. FPGA — 3.3V. Підключити I²C напряму не можна — FPGA отримає 5V на GPIO піни і може згоріти.

I²C — **open-drain** протокол. Обидва пристрої підключені до одних і тих самих двох дротів:

```
FPGA ──┬── SDA ──┬── Arduino
       │         │
      [R]       [R]   ← pull-up резистори до 3.3V
```

Щоб передати `1` — пристрій **нічого не робить**, відпускає лінію. Pull-up резистор сам підтягує до 3.3V.
Щоб передати `0` — пристрій **тягне лінію до GND**.

Тобто пристрої можуть тільки "тягнути вниз" або "відпускати" — ніхто не штовхає вгору, це робить резистор. Тому напруга HIGH на шині = напруга живлення pull-up резистора:

```
З pull-up до 5V:   пристрій відпускає → 5V на вході FPGA → 💥
З pull-up до 3.3V: пристрій відпускає → 3.3V на вході FPGA → ✅
                   пристрій тягне GND → 0V на вході FPGA  → ✅
```

**Рішення:** ADS1115 модуль має вбудовані pull-up резистори 10kΩ до 3.3V. Підключив SDA/SCL модуля до шини — вони тягнуть лінії до 3.3V. Arduino пише, FPGA читає нормально.
...
Проблема — **кеш процесора**. Компілятор оптимізує код і думає: "`smoothed` змінюється тільки в `loop()`, покладу її в регістр процесора щоб не лізти в RAM кожен раз":

```
RAM:     smoothed = 750  (реальне, оновлене ISR)
Регістр: smoothed = 512  (старе, кешоване компілятором)

loop() читає з регістру → бачить 512 замість 750
```

ISR завжди пише в RAM — але `loop()` про це не знає, бо читає зі свого регістру. Причому це не "запізнення на один цикл" — компілятор може закешувати назавжди, залежить від рівня оптимізації. Підступний баг: іноді працює, іноді ні.

`volatile` каже компілятору: **"не кешуй, завжди читай з RAM"**.

Для Java-розробника: той самий `volatile` що між потоками — тільки тут між `loop()` і ISR. Механізм ідентичний.
...
`volume_raw` — 16-бітне значення від Arduino. Але `analogRead()` повертає лише **10 біт** (0..1023) — старші 6 біт завжди 0. Тобто реально значущих даних там 10 біт, хоча тип 16-бітний.
...
`{1'b0, volume_raw[14:0]}` — захисний патерн: фіксуємо старший біт в 0. Технічно він і так 0, але якщо раптом ADS1115 поверне від'ємне значення через шум на I²C або глюк — без цього `$signed(vol)` стане від'ємним і множення інвертує звук або дасть переповнення. Діапазон `vol`: 0..32767.
